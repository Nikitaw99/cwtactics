(function() {
  ...
  function nextValidPosition(x, y, minValue, walkLeft, cb, arg) {
    ...
    y += mod;
    for (;
      (walkLeft) ? x >= 0 : x < maxLen; x += mod) {
      for (;
        (walkLeft) ? y >= 0 : y < maxLen; y += mod) {
  ...
  function forEach(callback) {
    ...
    for (var x = 0; x < e; x++) {
      for (var y = 0; y < e; y++) {
        callback(this.centerX + x, this.centerY + y, this.data[x][y]);
      }
    }
  };
  ...
  function map(callback) {
    ...
    for (var x = 0; x < e; x++) {
      for (var y = 0; y < e; y++) {
        this.data[x][y] = callback(this.centerX + x, this.centerY + y, this.data[x][y]);
      }
    }
  };
